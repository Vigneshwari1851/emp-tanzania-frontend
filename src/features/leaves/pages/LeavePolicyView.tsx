...
export function LeavePolicyView() {
  ...
  const handleDownloadOfficialDocument = async () => {
        ...
        <div className="flex items-center gap-2">
          {policy.document_url && (
            <Button
              ...
              onClick={handleDownloadOfficialDocument}
      ...
      <div className="flex-1 overflow-y-auto py-8 px-4">
        <div
          ...
          style={{
            ...
            borderRadius: '4px',
            padding: policy.document_url ? '16px' : '64px 80px',
            minHeight: policy.document_url ? 'auto' : '1000px',
          }}
        >
          {policy.document_url ? (
            policy.document_url.toLowerCase().endsWith('.pdf') ? (
              <iframe
                src={`${window.location.origin}${policy.document_url}`}
                className="w-full min-h-[750px] border-0 rounded bg-white shadow-inner"
                title={policy.policy_name}
              />
            ) : (
              /\.(jpg|jpeg|png|gif|webp)$/i.test(policy.document_url) ? (
                <div className="bg-card p-2 rounded max-w-full flex items-center justify-center">
                  <img
                    src={`${window.location.origin}${policy.document_url}`}
